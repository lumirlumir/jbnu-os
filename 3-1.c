...
static inline int count_valid(frame *pas, process* cur);		//count the number of valid frames.
static inline int count_references(frame *pas, process* cur);	//count the number of ref on each frames.
...
int main(int argc, char *argv[]) {
	...
	/* PAS Creation */ 
	frame *pas = (frame*)calloc(PAS_FRAMES, sizeof(frame));	//Create Pysical Address Space.
	int pas_idx = 0; //Free Frame Idx.

	/* PTE Insertion*/ //PAGETABLE_FRAMES * count_existing_process(&pro_q) : page table이 차지한 frame수.
	for(pas_idx; pas_idx < PAGETABLE_FRAMES * count_existing_process(&pro_q); pas_idx++) {
	...
	while(true) {
		...
		list_for_each_entry_reverse(cur, &pro_q, pro) {
			...
			/* declaration */
			pte* cur_pte = (pte*) &pas[cur->references[cur->pc] / 8 + (cur->pid * PAGETABLE_FRAMES)]; // (cur->references[cur->pc]) / 8 : frame 번호.
			int offset = (cur->references[cur->pc]) % 8; // (cur->references[cur->pc]) % 8 : offset 번호.

			/* Exit if OOM */
			if(pas_idx == PAS_FRAMES && cur_pte[offset].vflag == PAGE_INVALID) {printf("Out of memory!!\n"); break;}

			/* Test */
			//printf("[PID:%02d | REF:%03d] Page access %03d: ", cur->pid, cur->pc, cur->references[cur->pc]);

			/* Case 1 : Abnormal (Page Fault) */
			if(cur_pte[offset].vflag == PAGE_INVALID) {
				/* Test */
				//printf("PF, Allocated Frame %03d\n", pas_idx);

				cur_pte[offset].frame = pas_idx;
				cur_pte[offset].vflag = PAGE_VALID;
				cur_pte[offset].ref ++;
				pas_idx++;
			}
			/* Case 2 : Normal */
			else if(cur_pte[offset].vflag == PAGE_VALID) {
				//printf("Frame %03d\n", cur_pte[offset].frame);
				cur_pte[offset].ref ++;
			}
			/* Case 3 : Error */
			else printf("****Case Judgement Error!****\n");
		...
		/* Exit */
		if(pas_idx == PAS_FRAMES) {"Out of memory!!\n"; break;}
	}
	...
	list_for_each_entry_reverse(cur, &pro_q, pro) {
		printf("** Process %03d: Allocated Frames=%03d PageFaults/References=%03d/%03d\n", 
			cur->pid, 
			af = PAGETABLE_FRAMES + count_valid(pas, cur), 
			pf = count_valid(pas, cur), 
			ref = count_references(pas, cur));

		for(int i=(cur->pid * PAGETABLE_FRAMES); i < (cur->pid * PAGETABLE_FRAMES + 8); i++) {
			pte* cur_pte = (pte*) &pas[i];
			for(int j=0; j<8; j++)
				if(cur_pte[j].vflag == PAGE_VALID)
					printf("%03d -> %03d REF=%03d\n", (i*8)%(8*8)+j, cur_pte[j].frame, cur_pte[j].ref);
		}
...
static inline int count_valid(frame *pas, process *cur){
	int count = 0;
	for(int i = (cur->pid * PAGETABLE_FRAMES); i < (cur->pid * PAGETABLE_FRAMES + PAGETABLE_FRAMES); i++) {
		pte* cur_pte = (pte*) &pas[i];
		for(int j = 0; j < PAGETABLE_FRAMES; j++)
			if(cur_pte[j].vflag == PAGE_VALID) count++;
	}
	return count;
}
static inline int count_references(frame *pas, process* cur){
	int count = 0;
	for(int i = (cur->pid * PAGETABLE_FRAMES); i < (cur->pid * PAGETABLE_FRAMES + PAGETABLE_FRAMES); i++) {
		pte* cur_pte = (pte*) &pas[i];
		for(int j = 0; j < PAGETABLE_FRAMES; j++)
			if(cur_pte[j].ref != 0) count += cur_pte[j].ref;
	}
	return count;
}
